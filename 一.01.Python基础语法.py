...
# 注意：python赋值语句的执行过程是：首相把等号右边的表达式计算出来，然后在内存中寻找一个位置把值存储进去，最后创建变量并引用这个内存地址。
#      也就是说，【python变量并不直接存储值，而是存储了值的内存地址或引用，这也是变量类型随时可变的原因】。
...
# 身份运算符	各运算符优先级相同：is
# 成员运算符	各运算符优先级相同：in
# 逻辑运算符	布尔非(not) > 布尔与(and) > 布尔或(or)
# 赋值运算符	各运算符优先级相同：=、+=、-=、*=、/=
# ———————————————————————————————————————————————————————————————————————————
# 注意：python中没有++、--


# 由于在其他语言中我们也学过运算符，下面我们讲讲Python运算符中区别与其他运算符的例子：
# 如下：


# # ①算术运算符
# 加 +   # 加法/序列的拼接（序列：列表、元组、字符串）
# 减 -
# 乘 *   # 乘法/序列的重复
# 除 /
# 余 %
# 指数 **   (a**b  即a的b次方)
# 取整除 //  (a//b  即a/b的值向下取整)  # 如：-15//4=-4（向下取整）
...
#
# [2]序列（列表、元组、字符串）的比较：
# # 计较方法：逐个比较对应位置的元素，直到某个元素比较出大小，或者某一方先结束为止。
# # 注意：同种类型才能比较
#
# 'abcd' > 'abcc'  # 比ascii码
# # 'abcd' > 'abc'
...
#
# [3]集合的比较：【测试是否为真子集】
# {1,2,3} < {1,2,3,4}  # True，为真子集
# {1,2,3} < {1,2,4}  # False，不为真子集
...
# # ④成员测试运算符
# 3 in [1,2,3]  # True，测试3是否在列表中
# 5 in range(1, 10, 1) # True，测试5是否在range列表中
# 'abc' in 'abcqwer'  # True，测试子串是否在字符串中
# for i in (1,2,3)  # 循环的固定格式
...
# {1,2,3} & {3,4,5} = {3}  # & 交集
# {1,2,3} ^ {3,4,5} = {1,2,4,5}  # ^ 对称差集
# {1,2,3} - {3,4,5} = {1,2}  # 差集



# ⑥逻辑运算符
# and 和 or 的判断方法与其他语言类似，具有短路原则
# 但是与其他语言不同的是：python逻辑运算符返回的并不是True或者False，而是【返回能确定结果的最后一个表达式的值】。
#
# 3>5 and a>3  # False  # 左假，直接返回左面表达式的值：False
# 3>5 or a>3   # NameError  # 左假，判断右，右假，返回有表达式的值：报错
# 3 and 5      # 5  # 左真，判断右，右真，返回右表达式的值：5
# 3 and 5>2    # True  # 同理，返回右表达式的值：True
